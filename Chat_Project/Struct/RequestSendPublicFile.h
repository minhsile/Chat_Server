...
ref class RequestSendPublicFileStruct :public ChatStruct
{
public:
	String^ fileName;

	RequestSendPublicFileStruct() {

		fileName = nullptr;
	}
	virtual array<Byte>^ pack() override {
		List<Byte>^ byteData = gcnew List<Byte>();
		byteData->AddRange(BitConverter::GetBytes(int(ChatStruct::MessageType::RequestSendPublicFile)));
